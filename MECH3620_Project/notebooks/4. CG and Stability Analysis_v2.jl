...
begin
    airfoil_path = raw"C:\Users\Smith\OneDrive\Documents\GitHub\3620_project\Airfoil\NASA_SC_0614.txt"

    isfile(airfoil_path) || error("Airfoil file not found at: $airfoil_path")

    foil_w_root = read_foil(airfoil_path)
    foil_w_tip  = read_foil(airfoil_path)
end
...
begin
	    ...
	    W_payload_final = 8210 - 4 * 90
	    W_crew_final = 360.0
	    W_fuel_final = 9252.73967
...
# ╔═╡ dc65ebaa-cd46-4763-bf96-7210f6bc620b
    ...
    # this code plots CL-α curves for cruise and takeoff/landing conditions
# using Kirchhoff stall model with automatic stall angle
# herec
begin
    # ----- helper function to process VLM data and plot -----
    function plot_CL_alpha(vlms, mach, title_suffix)
        # Convert VLM results to DataFrame
        nffs = mapreduce(nearfield, hcat, vlms)'
        df = DataFrame([alphas nffs], [:al, :CDi, :CY, :CL, :Cl, :Cm, :Cn])
        
        # Kirchhoff parameters
        n_exp = 2.0
        α_stall_initial_deg = 20.0
        
        # Linear region for fit (-5° to 20°)
        small_range = (-5:20)
        mask = df.al .∈ Ref(small_range)
        α_small = df.al[mask]
        CL_small = df.CL[mask]
        slope = (CL_small[end] - CL_small[1]) / (α_small[end] - α_small[1])
        intercept = CL_small[1] - slope * α_small[1]
        CL_linear = intercept .+ slope * df.al
        
        # Kirchhoff factor function
        function kirchhoff_factor(α_deg, α_stall_deg, n)
            x = α_deg / α_stall_deg
            f = max(0.0, 1.0 - x^n)
            return sqrt(f)
        end
        ...
        factor1 = kirchhoff_factor.(α_deg, α_stall_initial_deg, n_exp)
        CL_corrected1 = CL_linear .* factor1
        max_CL1, idx_max = findmax(CL_corrected1)
        α_stall_determined_deg = α_deg[idx_max]
        
        # Second pass: final correction
        factor_final = kirchhoff_factor.(α_deg, α_stall_determined_deg, n_exp)
        CL_corrected_final = CL_linear .* factor_final
        max_CL_final, idx_max_final = findmax(CL_corrected_final)
        α_stall_final_deg = α_deg[idx_max_final]
        
        # Print results
        println("\n--- Mach = ", mach, " ---")
        println("Stall angle (α_stall) = ", α_stall_final_deg, "°")
        println("Maximum CL (CL_max)  = ", max_CL_final)
        println("-----------------------------------")
        
        # Create plot
        p = plot(df.al, CL_linear, label="VLM (linear, no stall)", lw=2, ls=:dash)
        plot!(p, df.al, CL_corrected_final, label="Kirchhoff corrected", lw=2, color=:red)
        vline!(p, [α_stall_final_deg], label="Stall α = $(α_stall_final_deg)°", ls=:dot)
        scatter!(p, [α_stall_final_deg], [max_CL_final],
                 label="CL_max = $(round(max_CL_final, digits=3)) at $(α_stall_final_deg)°",
                 color=:blue)
        xlabel!(p, "α (deg)")
        ylabel!(p, "CL")
        title!(p, "Lift Curve – $title_suffix (M = $mach)")
        return p
    end
    
    # ----- Generate plots for cruise and low-speed -----
    p_cruise = plot_CL_alpha(vlms_M1, M1, "Cruise Condition")
    p_low    = plot_CL_alpha(vlms_M2, M2, "Takeoff/Landing Condition")
    
    # ----- Display side‑by‑side -----
    plot(p_cruise, p_low, layout=(1,2), size=(1000, 450))
end
...
begin
	...
	function passenger_items_dict()
	    return Dict(
	        "pax_zone_$(z)" => (pax_per_zone_board[z] * W_pax_each, x_zone_board[z])
	...
	open(joinpath(outdir, "oei_vtail_check.txt"), "w") do io
	    ...
	    println(io)
	        ...
	        savefig(loading_cases_plot, joinpath(outdir, "loading_cases_cg_curve.pdf"))
	        savefig(mission_cg_plot, joinpath(outdir, "mission_cg_excursion.pdf"))
	        savefig(mission_only_plot, joinpath(outdir, "mission_only_cg_excursion.pdf"))
	        savefig(potato_plot, joinpath(outdir, "boarding_potato_plot.pdf"))
	        savefig(sm_loading_plot, joinpath(outdir, "static_margin_loading_cases.pdf"))
	        savefig(sm_mission_plot, joinpath(outdir, "static_margin_mission.pdf"))
	        savefig(cg_excursion_plot, joinpath(outdir, "cg_excursion_envelope_plot.pdf"))
	        savefig(weight_balance_plot, joinpath(outdir, "weight_balance_component_locations.pdf"))
	        savefig(potato_points_plot, joinpath(outdir, "boarding_potato_points_only.pdf"))
	        savefig(potato_plot, joinpath(outdir, "zone_boarding_potato_plot.pdf"))
	savefig(aircraft_wb_plot, joinpath(outdir, "aircraft_weight_balance_3d.pdf"))
